...
#define KEYFRAME_EVENT_INTERVAL 2
#define FIRST_KEY_FRAME 1
...
void putEventMetadataMKVTags(CustomData *data, bool is_start_event, uint64_t event_timestamp)
{
    ...
    eventMetadata->numberOfPairs = (is_start_event) ? 3 : 2;
    ...
    eventMetadata->names[1] = strdup(event_timestamp_name.c_str());
    eventMetadata->values[1] = strdup(event_timestamp_value.c_str());

    if (is_start_event)
    {
        std::string event_triggered_by_name = "VIDEO_INTERVAL_TRIGGERED_BY";
        std::string event_triggered_by_value = "PIR";

        eventMetadata->names[2] = strdup(event_triggered_by_name.c_str());
        eventMetadata->values[2] = strdup(event_triggered_by_value.c_str());
    }
